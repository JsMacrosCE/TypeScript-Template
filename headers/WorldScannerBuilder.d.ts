...
declare namespace TypedWorldScannerBuilder {
    ...
    type BooStrNumMethod<T> = {
        [K in keyof T]: K extends 'hashCode'
            ? never
            : T[K] extends (() => infer R extends boolean | string | number)
    ...
    type FilterFor<M> = M extends (() => infer R extends boolean | string | number)
        ? IsStrictAny<R> extends false
            ? [R] extends [infer S extends string]
                ? StringFilter<S>
                : R extends number
                  ? NumberFilter
                  : R extends boolean
    ...
    type Contains<T extends string, S extends string> =
        | { [A in S]: T extends `${string}${A}${string}` ? A : never }[S]
        | T;
    ...
    type StringCompareTo<M extends StringFilterOperation, T extends string, S extends string> = M extends 'EQUALS'
        ? T
        : M extends 'CONTAINS'
          ? Contains<T, S>
          : M extends 'STARTS_WITH'
            ? StartsWith<T, S>
            : M extends 'ENDS_WITH'
              ? EndsWith<T, S>
              : string;
