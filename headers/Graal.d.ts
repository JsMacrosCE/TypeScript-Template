...
declare function loadWithNewGlobal(
    source: string | Packages.java.io.File | Packages.java.net.URL,
    ...arguments: any[]
): any;
...
declare namespace Polyglot {
    ...
    export type { _import as import, _export as export };
}
...
declare namespace Java {
    ...
    export function to<T extends JavaArray<any>>(
        jsArray: GetJava.JavaToJsArr<T>,
        toType: JavaClass<T> | (new (length: int) => T)
    ): T;
    export function to<T extends `${JavaTypeList | keyof GetJava.Primitives}[]`>(
        jsArray: GetJava.GraalJsArr<T>,
        toType: T
    ): GetJava.GraalTo<T>;
...
type ListPackages<T extends object, P extends string = ''> = IsStrictAny<T> extends true
    ? never
    : T extends JavaClassArg
      ? P
      : { [K in keyof T]: ListPackages<T[K], P extends '' ? K : `${P}.${K}`> }[keyof T];

namespace GetJava {
    type Type$Graal<P extends string, _K extends keyof Primitive<any> = 'type'> = IsStrictAny<P> extends true
        ? any
        : P extends `${string}[]`
          ? ArrayType<P> extends JavaArray<infer T>
              ? typeof Packages.java.lang.Array<T>
              : never
          : P extends keyof Primitives
            ? Primitives[P][_K]
            : Type<P>;

    type ArrayType<P extends string, _K extends keyof Primitive<any> = 'type'> = P extends `${infer C extends string}[]`
        ? JavaArray<ArrayType<C>>
        : Type$Graal<P, _K> extends { readonly class: JavaClass<infer T> }
          ? T
          : unknown;
    ...
    type Type$Reflection<P extends string> = P extends keyof Primitives ? Primitives[P]['type']['class'] : TypeClass<P>;

    type TypeClass<T> = Type<T>['class'] extends infer C extends JavaClass<any>
        ? IsStrictAny<C> extends false
            ? C
            : unknown
        : unknown;

    type Type<P extends string, T extends object = typeof Packages> = IsStrictAny<T> extends true
        ? unknown
        : P extends `${infer K}.${infer R}`
          ? Type<R, T[K]>
          : P extends ''
            ? T extends (abstract new (...args: any[]) => any) & { readonly class: JavaClass }
                ? T
                : unknown
            : Type<'', T[P]>;
...
declare namespace Packages {
    namespace java {
        namespace lang {
            ...
            interface Class<T>
                extends io.Serializable,
                    reflect.GenericDeclaration,
                    reflect.Type,
                    reflect.AnnotatedElement,
                    invoke.TypeDescriptor$OfField<Class<T>>,
                    constant.Constable {
                new (...args: any[]): T;
            }
            class Class<T> extends Object {
                ...
                static forName<C extends string>(
                    name: C,
                    initialize: boolean,
                    loader: ClassLoader
                ): GetJava.TypeClass<C>;
                static forName<C extends JavaTypeList>(
                    name: C,
                    initialize: boolean,
                    loader: ClassLoader
                ): GetJava.TypeClass<C>;
            ...
            export type { Class, Object, Interface, Comparable, Array, StackTraceElement, Throwable, Iterable };
        }
        ...
        namespace util {
            ...
            export type { Collection, List, Map, Set };
        }
